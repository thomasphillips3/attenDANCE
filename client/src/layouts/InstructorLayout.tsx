...
/**
 * InstructorLayout -- auth + role gate for the instructor portal.
 *
 * Auth check:
 * - Loading: spinner
 * - No session: redirect to /login
 * - Non-instructor role: redirect to / (front desk) or /admin
 * - Instructor: render top nav + child routes via Outlet
 *
 * Role is decoded from JWT app_metadata (server-writable only).
 * This is a UX gate -- every API endpoint independently checks role.
 *
 * Design: 18px+ body text, 56px+ tap targets, Atkinson Hyperlegible body,
 * studio purple branding. Modeled after ParentLayout.
 */

const NAV_ITEMS = [
  { path: '/instructor', label: 'My Classes', exact: true },
  { path: '/instructor/schedule', label: 'Schedule' },
  { path: '/instructor/hours', label: 'Hours' },
]
...
export default function InstructorLayout() {
  ...
  if (role !== 'instructor') {
    // Non-instructor users trying to access instructor portal
    if (role === 'admin') {
